    ...
    return (<nav className="fixed top-0 left-0 right-0 z-50 p-4 md:p-6 flex items-center justify-between pointer-events-none">
      ...
      <div className="pointer-events-auto flex items-center gap-3">
        {isAuthenticated ? (<div className="flex items-center gap-3 bg-white/80 backdrop-blur-md px-4 py-2 rounded-full border border-white/80 shadow-md">
            ...
            <button onClick={() => openAuthModal("Sign in to access your account & cart.")} className="px-5 py-2.5 rounded-full bg-white/90 text-[#2F5D34] font-bold text-xs uppercase tracking-wider border border-[#2F5D34]/20 shadow-md hover:bg-white transition-all">
              Sign In
            </button>
            <button onClick={() => openAuthModal("Join KLN Ayurveda for personalized wellness.")} className="px-5 py-2.5 rounded-full bg-[#2F5D34] text-white font-bold text-xs uppercase tracking-wider shadow-lg hover:bg-[#224426] transition-all">
              Register
            </button>
          </div>)}
      </div>
