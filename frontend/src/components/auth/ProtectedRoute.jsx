...
export default function ProtectedRoute({ children, pageTitle = "this section" }) {
    ...
    }, [isMounted, isAuthenticated, openAuthModal, pageTitle]);

    if (!isMounted) {
        return (
            <div className="min-h-screen w-full flex items-center justify-center bg-[#F7F4EC]">
                <div className="text-center">
                    <span className="text-4xl animate-bounce">🌿</span>
                    <p className="mt-2 text-sm font-bold text-[#2F5D34]">Loading...</p>
                </div>
            </div>
        );
    }
