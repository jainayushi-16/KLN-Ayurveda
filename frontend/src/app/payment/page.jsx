...
export default function PaymentPage() {
  ...
  const { items: cartItems, totalItems: totalItemsCount, subtotal, updateQuantity, removeItem, clearCart } = useCartStore();
  ...
  const handlePayNow = async () => {
    ...
    setTimeout(async () => {
      ...
      const order = await placeOrder(cartItems, grandTotal, paymentDetails);
        ...
        <section className="pt-12 pb-8 w-full px-6 md:px-12 lg:px-16 relative z-10">
          <div className="max-w-[1800px] mx-auto flex flex-col md:flex-row items-center justify-between gap-6 border-b border-[#2F5D34]/15 pb-8">
            ...
            <div className="flex items-center gap-3 text-xs font-bold uppercase tracking-wider">
              <Link href="/cart" className="text-gray-400 hover:text-[#2F5D34]">
                1. Cart
              </Link>
              <span className="text-gray-300">→</span>
              <Link href="/checkout" className="text-gray-400 hover:text-[#2F5D34]">
                2. Shipping
              </Link>
              <span className="text-gray-300">→</span>
          ...
          <div className="max-w-[1800px] mx-auto">
            {cartItems.length === 0 ? (
              <div className="text-center py-24 bg-white/80 backdrop-blur-xl rounded-[2.5rem] border border-dashed border-[#2F5D34]/30 max-w-xl mx-auto shadow-sm">
                ...
                <h2 className="text-2xl font-bold text-[#222123] mb-3">Your Cart is Empty</h2>
                <p className="text-gray-600 text-sm mb-6">No items left in cart to process payment.</p>
                <Link href="/shop">
              ...
              <div className="flex flex-col lg:flex-row gap-10 items-start">
                ...
                {/* Right Column: Final Summary & In-Summary Controls */}
